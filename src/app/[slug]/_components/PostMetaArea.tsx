...
const PostMetaArea: React.FC<PostMetaAreaProps> = ({ post }) => {
	return (
		<section className="mb-8">
			<Link href={`/${isValidCategory(post.category) ? CATEGORY_PATH_MAP[post.category].path : "dev"}`} className="w-fit">
			...
			<div className="flex justify-between gap-y-3 flex-col sm:flex-row">
				...
				<p className="text-neutral-600 text-sm">{dayjs(post.date).format("YYYY-MM-DD")}</p>
			</div>
			<hr className="mt-8" />
		</section>
	);
