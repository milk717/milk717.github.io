...
export const getAllPostByCategory = (category: string) => {
	return allPosts.filter((post) => post.category === category).sort((post1, post2) => (post1.date > post2.date ? -1 : 1));
};

export const getAllPostByTag = (tag?: string) => {
	return allPosts.filter((post) => (!tag ? true : post.tags?.includes(tag))).sort((post1, post2) => (post1.date > post2.date ? -1 : 1));
};
...
export const getPostListByKeyword = (keyword: string) => {
	...
	return allPosts.filter(
		(post) => post.title.match(regExp) || post.tags.some((tag) => tag.match(regExp)) || post.excerpt?.match(regExp) || post.date.match(regExp)
	);
};
